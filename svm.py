...
def test_SVM(face_profile_data, face_profile_name_index, face_dim, face_profile_names):
    """
    Testing: Build the SVM classification model using the face_profile_data matrix (numOfFace X numOfPixel)
    and face_profile_name_index array, face_dim is a tuple of the dimension of each image(h,w)
    Returns the SVM classification model
    Parameters
    ----------
  ...
  tol=0.001, verbose=False)
   ...
   # savefig("confmat.png", format="png")
    ...
    def title(y_pred, y_test, face_profile_names, i):                     #title() ==>>       predicted:name,
        pred_name = face_profile_names[y_pred[i]].rsplit(' ', 1)[-1]      #                   true:name
...
print(face_profile_data.shape,face_profile_name_index.shape)

# Build the classifier
#clf, pca = svm.build_SVC(face_profile_data, face_profile_name_index, FACE_DIM)
clf, pca = test_SVM(face_profile_data, face_profile_name_index, FACE_DIM, face_profile_names)
